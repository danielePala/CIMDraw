...
function cimDiagramController() {
	...
	render: function(diagramName) {
	    ...
	    // Connectivity Nodes
	    if (diagram.length > 0) {
		allConnectivityNodes = this.filterConnectivityNodes(allConnectivityNodes, edges, ceEdges, ioEdges, doEdges);
	    }
	    ...
	    let termsToDraw = ceEdges.filter(ceEdge => nodes.indexOf(ceEdge.source) > -1).map(ceEdge => ceEdge.target);
	    edges = edges.filter(edge => termsToDraw.indexOf(edge.target) > -1); // TODO: we may need busbar sections to draw connectivity nodes
	...
	filterConnectivityNodes: function(allConnectivityNodes, edges, ceEdges, ioEdges, doEdges) {
